...
import options
######### parser ###########
opt = options.Options().init(argparse.ArgumentParser(description='Image demoreing')).parse_args()
print(opt)
...
######### Logs dir ###########
log_dir = os.path.join(opt.save_dir, 'demoering', opt.dataset, opt.arch+opt.env)
    ...
    for i, data in enumerate(tqdm(train_loader), 0): 
        ...
        if (i+1)%eval_now==0 and i>0:
            with torch.no_grad():
                ...
                print("[Ep %d it %d\t PSNR Demoire: %.4f\t] ----  [best_Ep_Demoire %d best_it_Demoire %d Best_PSNR_Demoire %.4f] " % (epoch, i, psnr_val_rgb,best_epoch,best_iter,best_psnr))
                with open(logname,'a') as f:
